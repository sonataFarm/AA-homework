...
def pulp_fiction_actors
  ...
  # get all castings with movie_id = pulp_fiction id
  # join to actors on actor_id
  movie_id = Movie.find_by(title: "Pulp Fiction").id
...
def uma_movies
  ...
  uma_id = Actor.find_by(name: 'Uma Thurman').id

  Movie.select('movies.id, title, yr').joins(:castings, :actors).where('actors.id = ?', uma_id).order('yr ASC').uniq
end
