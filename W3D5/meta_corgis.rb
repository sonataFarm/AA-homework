...
class MetaCorgiSnacks
  ...
  def self.define_snack(*types)
    types.each do |type|
      define_method(type) do
        info = @snack_box.send("get_#{type}_info", @box_id)
        tastiness = @snack_box.send("get_#{type}_tastiness", @box_id)
        result = "#{type.upcase}: #{info}: #{tastiness} "
        tastiness > 30 ? "* #{result}" : result
      end
    end
  end

  define_snack :bone, :kibble, :treat

  # def method_missing(method_name, *args)
  #   component = method_name.to_s
  #
  #   taste_method_name = "get_#{method_name}_tastiness".to_sym
  #   tastiness = @snack_box.send(taste_method_name, @box_id)
  #
  #   info_method_name = "get_#{method_name}_info".to_sym
  #   info = @snack_box.send(info_method_name, @box_id)
  #
  #   result = "#{component.capitalize}: #{info}: #{tastiness} "
  #   tastiness > 30 ? "* #{result}" : result
  # end
